...
from process import start_configurator, condition_is_met, process_choice, translate
            ...
            if condition_is_met(c['condition']):
                ...
                if c['type'] == 'choice':
                    ...
                    for i, choice in enumerate(c['meta']['choices']):
                        with cols[i]:
                            st.button(translate(choice['title']), on_click=process_choice, args=(c['id'], choice['value']))
                if c['type'] == 'input':
                    st.number_input(translate(c['meta']['label']), placeholder=translate(c['meta']['placeholder']), min_value=c['meta']['range']['min'], max_value=c['meta']['range']['max'])
